...
namespace WCFParseApp
{
            ...
            if (argv.Length == 2){
                try{
                    ...
                    if (argv[0].ToLower().Equals("encode")){
                        string base64 = Convert.ToBase64String(NBFS.EncodeBinaryXML(Encoding.UTF8.GetString(Convert.FromBase64String(argv[1]))));
                        Console.WriteLine(base64);
                    }
                    else{
                        //序列化的请求参数的base64
                        byte[] requestParams = Convert.FromBase64String(argv[1]);
                        try
                        {
                            string url = "http://qxy.yadu.com/ClientBin/ZyCRM-Web-Service-Pub-PpService.svc/binary/getListAll";
                            WebClient wc = new WebClient();
                            // 采取POST方式必须加的Header
                            wc.Headers.Add("Content-Type", "application/msbin1");
                            byte[] result = wc.UploadData(url, "post", requestParams);

                            string requestbase = Convert.ToBase64String(result);
                            Console.WriteLine(Convert.ToBase64String(Encoding.UTF8.GetBytes(NBFS.DecodeBinaryXML(result))));
                            Console.Read();
                        }
                        catch (Exception e){
                            Console.WriteLine("---->GetWebPage: " + e.StackTrace);
                        }
                ...
                Console.WriteLine(Convert.ToBase64String(Encoding.UTF8.GetBytes("Usage: NBFS [encode|decode] Base64Data\n\nNOTE: All output, including exceptions, will be returned as a Base64 string.")));
            }
        }
    }
    ...
    public class NBFSNet
    {
        ...
        public byte[] EncodeBinaryXML(byte[] xml)
        {
    ...
    public class WcfBinaryCodec
    {
        ...
        public byte[] EncodeBinaryXML(byte[] xmlInput)
        {
